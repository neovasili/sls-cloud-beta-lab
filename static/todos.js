...
const TodoRow = ({ item, setItems, fetchTodos, updateTodo }) => {
  ...
  const deleteTodo = async (item) => {
    ...
    try {
      ...
      const result = await fetch(`/api/v1/todos/${item.id}`, {
        method: "DELETE",
        headers: {
...
const Todos = () => {
  ...
  const updateTodo = async (item) => {
    ...
    const result = await fetch(`/api/v1/todos/${data.id}?status=incomplete`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json"
      },
  ...
  const fetchTodos = async () => {
    ...
    try {
      let todos = await fetch(`/api/v1/todos?status=${status}`);
      let curretStage = await fetch(`/api/v1/stages`);

      curretStage = await curretStage.json();

      console.log(`Current stage: ${curretStage.stage}`);
